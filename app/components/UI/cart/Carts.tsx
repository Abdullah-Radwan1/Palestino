...
const Carts = () => {
 ...
 const selectProducts = useSelector((state: Rootstate) => state.cart.cartItems);
 const totalAmount = useSelector((state: Rootstate) => state.cart.totalPrice);
 const dispatch = useDispatch();
 const toggle = () => {dispatch(toggleUIcart())};
     ...
     <div className="cart">
      ...
      <div className="added-product">
       {selectProducts.length === 0 ? (
        <h3 className="m-auto text-center pt-32">there is no items yet</h3>
       ) : (
        selectProducts.map((item) => <CartProduct key={item.id} item={item} />)
       )}
      </div>
