  ...
  <div className="FoodDetails mt-20">
    ...
    <div className="images flex justify-center items-center gap-x-16">
     {product && (
      <div className="three-img img flex flex-col gap-y-10">
       <Image onClick={() => setShowedImg(product.image01)} alt="" src={product.image01} />
       <Image onClick={() => setShowedImg(product.image02)} alt="" src={product.image02} />
       <Image onClick={() => setShowedImg(product.image03)} alt="" src={product.image03} />
      </div>
     )}
    ...
    {tab === "desc" ? (
     <div className="description">
      <div className="desc w-[70%] text-slate-500">
       Lorem ipsum dolor sit, amet consectetur adipisicing elit. Error iusto deserunt animi, sapiente sit saepe praesentium architecto tempora sint rem.
      </div>
     </div>
    ) : (
   ...
   <div className="container all-products">
    {relatedProducts.map((product) => (
     <ProductCard key={product.id} id={product.id} title={product.title} image01={product.image01} price={product.price} />
    ))}
   </div>
  </div>
 )
